...
class PlaneRenderer(val lifecycle: ArSceneLifecycle) : ArSceneLifecycleObserver {
    ...
    override fun onArFrame(arFrame: ArFrame) {
        if (isEnabled) {
            try {
                ...
                val hitResult =
                    getHitResult(arFrame.frame, renderer.desiredWidth, renderer.desiredHeight)
                ...
                val updatedPlanes = arFrame.session.allPlanes
    ...
    private suspend fun loadPlaneMaterial() {
        val texture = Texture.builder()
            .setSource(
                renderer.context,
                Uri.parse("textures/trigrid_2.png")
            )
            .setSampler(
    ...
    companion object {
        ...
        private const val DEFAULT_TEXTURE_WIDTH = 512f
        private const val DEFAULT_TEXTURE_HEIGHT = 512f
